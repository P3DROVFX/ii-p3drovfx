...
NEW_QML_FILES = {
    "mediaCircleWidget": "AndroidMediaCircleToggle.qml",
    "expressiveMediaWidget": "AndroidExpressiveMediaToggle.qml",
    "cdMediaWidget": "AndroidCdMediaToggle.qml",
    "compactMediaWidget": "AndroidCompactMediaToggle.qml",
    "nothingRingMediaWidget": "AndroidNothingRingMediaToggle.qml"
}
...
def check_braces_balance(path):
    ...
    # Character-by-character scanner ignoring strings and comments
    i = 0
    n = len(content)
    counts = {"{": 0, "}": 0, "(": 0, ")": 0, "[": 0, "]": 0}
...
def test_catalog():
    ...
    for variant in MEDIA_VARIANTS:
        pattern = rf"{variant}:\s*\{{[^}}]*variantGroup:\s*\"media\""
        if not re.search(pattern, content, re.DOTALL):
            print(f"FAIL: {variant} not configured with variantGroup: 'media' in TOGGLE_TYPES")
            return False
        print(f"PASS: {variant} has variantGroup: 'media' in TOGGLE_TYPES")

    # Check canonical mappings
    aliases = [
        ("media", "mediaWidget"),
        ("mediaCircle", "mediaCircleWidget"),
        ("expressiveMedia", "expressiveMediaWidget"),
        ("cdMedia", "cdMediaWidget"),
        ("compactMedia", "compactMediaWidget"),
        ("nothingRingMedia", "nothingRingMediaWidget"),
    ]
    for alias, target in aliases:
        if alias not in content or target not in content:
            print(f"FAIL: alias {alias} -> {target} not found in canonicalType()")
            return False
    # Check expressiveMediaFootprints
    if "function expressiveMediaFootprints()" not in content:
        print("FAIL: expressiveMediaFootprints() missing in QuickToggleCatalog.js")
        return False
    print("PASS: expressiveMediaFootprints() defined in QuickToggleCatalog.js")
...
def test_chooser():
    ...
    if "import qs.modules.common.quickToggles.androidStyle.media" not in content:
        print("FAIL: media directory import missing in AndroidToggleDelegateChooser.qml")
        return False
    print("PASS: media import present in AndroidToggleDelegateChooser.qml")
...
def main():
    ...
    if passed:
        print("\nAll Media Quick Toggle contract tests passed successfully!")
        sys.exit(0)
    else:
        print("\nSome tests failed.")
        sys.exit(1)
