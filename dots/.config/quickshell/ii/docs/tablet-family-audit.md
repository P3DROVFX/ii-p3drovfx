...
> **Data:** 2026-09-01 · **Branch:** `agent/tablet-family-base` · **HEAD:** `980fbe8b0`
...
| **OSK** | 🔴 Fraco | 3/10 | Auto-show desligado, binário não compilado, **desativada na lock screen**. |
...
| Evidência | Arquivo |
|---|---|
| Deck (OSK padrão) não renderiza com a tela bloqueada | `modules/ii/onScreenKeyboard/DeckWindow.qml:38` — `visible: !GlobalStates.screenLocked` |
| OSK clássica idem | `modules/ii/onScreenKeyboard/OnScreenKeyboard.qml:59` |
| O helper de auto-show é desligado ao bloquear | `services/OskAutoShow.qml` — `running: root.enabled && !GlobalStates.screenLocked` |
| A lock screen só tem campo de texto — sem keypad, sem botão de teclado | `modules/ii/lock/LockSurface.qml:579` (`ToolbarTextField`); `grep -l keypad modules/ii/lock/` → vazio |
...
**Duas correções, e as duas valem:**

1. **Liberar a OSK sobre a lock screen em família touch-first.** A condição
   `!GlobalStates.screenLocked` vira `!GlobalStates.screenLocked || PanelFamily.touchFirst`,
   e o layer da OSK precisa ficar acima da superfície de lock. Isto é uma mudança em
   `modules/ii/`, então segue a regra do §3 do plano: ou vira capability lida de
   `PanelFamily`, ou a OSK é promovida para `modules/common/`. **Promover é o certo** — as
   três famílias já instanciam o mesmo `OnScreenKeyboard`.
2. **PIN pad numérico na lock screen**, oferecido quando a família é touch-first. É o que
   Android e One UI usam por padrão; um teclado completo para digitar uma senha de
   desbloqueio é a solução de desktop. Alvos de 72px, sem preview de caracteres.

> ⚠️ **Cuidado de segurança:** uma OSK sobre a lock screen precisa ser uma superfície que
> não vaza — sem histórico, sem sugestões, sem clipboard, e o `Ydotool.releaseAllKeys()` que
> já existe no `onActiveChanged` tem que continuar rodando. Verificar também que a OSK não
> pode ser usada para alcançar atalhos globais enquanto bloqueado.
...
| Evidência | Onde |
|---|---|
| Desligada por padrão | `modules/common/Config.qml:3917` — `property bool enable: false` |
| O binário não existe — só o fonte | `ls scripts/osk/` → `osk_autoshow_src/`, `README.md`. Sem `osk_autoshow` |
| Nenhuma UI de build | `TouchGesturesConfig.qml:74` tem `codeSnippet` para compilar o `touch_gestures`; a página de OSK não tem equivalente |
...
`TabletNavigation.home()` despacha `workspace = 'empty'`
(`modules/tablet/navigation/TabletNavigation.qml:60`). Isso vai para o **primeiro workspace
...
monitor (`1`, ou o menor id do monitor) é a home. `home()` vai para ele; um segundo toque em
Home (como no Android) vai para a gaveta. Adicionar da gaveta escreve no home workspace, não
no focado. Isso também conserta o indicador de páginas, que hoje conta workspaces
arbitrários em vez de páginas de home screen.

### B4 — Recentes não mostra os recentes 🔴

`TabletRecentsContent.qml:38–47`:
...
`HyprlandData.windowList` (`GlobalStates.qml:88`, `OverviewWidget.qml:174`,
`WindowManagementPanel.qml:70`, `TilingAssistant.qml:786`). `focusHistoryID === 0` é a janela
...
Três famílias, uma troca a quente que funciona (`PanelFamilyLoader` em `shell.qml:117–142`
recarrega por URL), e **nenhuma UI**:

| Caminho | Onde | Descoberto por |
|---|---|---|
| `qs -c ii ipc call panelFamily cycle` | `shell.qml:215` | Ler o AGENTS.md |
| Keybind `panelFamilyCycle` | `shell.qml:223` | Ler o AGENTS.md |
| Settings | — | **não existe** |
| Welcome | — | **não existe** |
...
| Elemento | Estado | Arquivo |
|---|---|---|
| Carrossel horizontal | ✅ | `TabletRecentsContent.qml` |
| Snapshot por card | ✅ `ScreencopyView` com `live: false` | `TabletRecentCard.qml:76` |
| Ícone + título | ✅ mas **acima do snapshot, dentro do card** | `TabletRecentCard.qml:47–68` |
| Arrastar p/ cima fecha | ✅ com threshold generoso | `TabletRecentCard.qml:99–133` |
| Toque foca | ✅ com `deferredRequested` (bom achado do plano) | `TabletRecentsWindow.qml:66–95` |
| "New workspace" | ✅ | `TabletRecentsContent.qml:141` |
| Estado vazio | ✅ `PagePlaceholder` | `TabletRecentsContent.qml:126` |
...
| **Split screen** | ❌ | — |
| **Screenshot / Select** | ❌ | — |
| **Taskbar visível** | ❌ dock é `WlrLayer.Top`, recentes é `Overlay` | `TabletDockWindow.qml:257` vs `TabletRecentsWindow.qml:38` |
...
casamento que `GlobalStates.qml:88` já faz. Depois de montar a lista, posicionar
`carousel.contentX` no card do app atual (índice 0 ou 1), não em zero. Sem isso, num tablet
com dez janelas o app que você acabou de sair está fora da tela.
...
As três últimas já existem como ações do `TouchGestureActionRegistry` (`toggleFloating`,
`toggleFullscreen`, `closeWindow`); é reuso, não código novo.
...
fazer*. Na prática está pior que "a fazer": os defaults de `Config.qml:3714–3715` são
...
do Home. A ação `back` já existe e já é vinculável (`TouchGestureActionRegistry` + o handler
que `TabletSystemKeybinds` instala) — falta só ser o **default da família**.
...
| Sobre a lock screen | ❌ **explicitamente bloqueada** (B1) |
...
│                                                          │
│                    Shell                                 │
│         Escolha a interface deste dispositivo            │
│                                                          │
│   ┌───────────┐   ┌───────────┐   ┌───────────┐          │
│   │ [preview] │   │ [preview] │   │ [preview] │          │
│   │           │   │           │   │           │          │
│   │illogical- │   │  Tablet   │   │  Waffle   │          │
│   │ impulse   │   │           │   │           │          │
│   │ ● atual   │   │           │   │           │          │
│   └───────────┘   └───────────┘   └───────────┘          │
│    Desktop        Touchscreen      Windows-like          │
│                                                          │
│        Toque para trocar · Esc para cancelar             │
└──────────────────────────────────────────────────────────┘
```
...
- **Cartões, não botões de ícone.** Trocar de família muda a tela inteira; um ícone de 64px
  não diz o que vai acontecer. Uma miniatura por família (asset estático, não screencopy)
  responde "como vai ficar" antes do commit.
...
- **Marca a atual e não recarrega ao tocar nela.** `Config.options.panelFamily = X` com o
  valor que já está lá é um no-op, mas o usuário não sabe disso — o cartão atual tem que
  estar visivelmente marcado e fechar sem trocar.

### 5.2 Onde ele é alcançado

| Entrada | Por quê |
|---|---|
| **Settings → uma página nova, "Shell"** | Onde alguém procura primeiro. Deve ser a **primeira** página, acima de tudo, porque decide o que as outras páginas configuram. |
...
| **Welcome, na página de experiência** | Um primeiro boot que não pergunta isso escolhe pelo usuário. `WelcomeExperiencePage.qml` é o lugar. |
| **Ação vinculável** `panelFamily` no `TouchGestureActionRegistry` | Hoje o `panelFamilyCycle` é um `GlobalShortcut` solto, fora do registry — então não pode ser vinculado a gesto nem aparece no seletor de ações do Settings. Corrigir. |
| **`SessionScreen`** | Um botão "Trocar interface" ao lado de Lock/Sleep/Logout, já que a metáfora é a mesma: mudar o que a sessão é. |

### 5.3 Cuidados

- **A troca é a quente e funciona** (`PanelFamilyLoader`, `shell.qml:117`), mas destrói e
  reconstrói todas as superfícies. Precisa de um estado de transição visível — a tela não pode
  simplesmente piscar por 1–2s sem explicação.
...
| Split 3 apps / grade | One UI | — | 🔵 Não fazer — o tiling do Hyprland já cobre, sem UI dedicada |
...
| Deslizar para dispensar | ✅ | ✅ `NotificationItem.qml:82–110` | ✅ |
| Agrupar por app | ✅ | ✅ (visível no print) | ✅ |
...
| Mídia na shade | ✅ | ✅ | ✅ |
| Notificação de progresso (download, etc.) | ✅ | ⚠️ depende do app | 🔵 |
...
| Biometria | ✅ | ⚠️ `fprintd` via polkit, não na lock | 🟡 |
...
| O relógio central está renderizado como dois blocos sobrepostos (`22`/`39`) e disputa atenção com o widget grande de relógio no canto superior direito do wallpaper. Dois relógios em telas de 1920px de largura. | 🔵 |
...
| Achado | Severidade |
|---|---|
| **~12–13 colunas.** `tileWidth` é `clamp(96, 148, width/8)` (`TabletAppDrawerContent.qml:52`) → 148px em 1920. O Pixel Tablet usa 6. Isso é densidade de menu de desktop, não de gaveta de tablet. | 🟠 |
| Ícones a `tileWidth * 0.52` ≈ 77px em placas de 148px — a placa é grande e o glifo é pequeno, o mesmo problema que o commit `980fbe8b0` corrigiu na bar. | 🟠 |
...
| Nomes truncados em duas linhas ("Alienware Command Center", "Calf Plugin Pack for JACK") empurram a linha inteira. | 🔵 |
| Apps duplicados visíveis no print (ChatGPT ×2, Cockpit Tools ×3, Contacts ×2, Cisco ×2). Não é problema da tablet — é o índice de `.desktop` — mas numa grade de toque é mais visível e mais caro de errar. | 🟡 |
...
ferramentas hospedadas, não só o file browser. **Recomendação:** em vez de adaptar cada uma,
adicionar ao contêiner de ferramentas uma capability `touchFirst` que as ferramentas leem
para esconder rodapés de atalho e trocar densidade — uma mudança, N ferramentas.
...
| A coluna de notificações usa ~50% da largura para 3 notificações, enquanto a de toggles está apertada. Uma arbitragem de espaço por ocupação (o projeto já tem `SpaceArbitration.js`) resolveria. | 🔵 |
| O rodapé "6 apps are active" com chevron é bom (é o padrão do Android 14+), mas está do lado da coluna errada — no Android fica junto às notificações. | 🔵 |
| Não há gesto para ir da shade a quick settings expandido — no Android um segundo puxão expande. Aqui já vem tudo expandido, o que é a escolha certa para tablet. | ✅ |
...
| # | Item | Sev | Esforço | Arquivos principais |
|---|---|---|---|---|
| 1 | OSK utilizável sobre a lock screen | 🔴 P0 | M | `ii/onScreenKeyboard/{DeckWindow,OnScreenKeyboard}.qml`, `ii/lock/LockSurface.qml`, `common/PanelFamily.qml` |
| 2 | PIN pad na lock screen em família touch-first | 🔴 P0 | M | `ii/lock/LockSurface.qml` (+ novo `LockKeypad.qml`) |
| 3 | Auto-show da OSK ligada por padrão + build do helper na UI | 🔴 P0 | P | `common/Config.qml:3917`, `settings/configs/TabletConfig.qml`, `scripts/osk/` |
| 4 | Recentes em ordem MRU + rolar até o app atual | 🔴 P0 | P | `tablet/recents/TabletRecentsContent.qml:38` |
| 5 | Home workspace determinístico | 🔴 P0 | M | `tablet/navigation/TabletNavigation.qml:60`, `tablet/homeScreen/TabletHomeIcons.qml` |
| 6 | Seletor de panel family (superfície + entradas) | 🔴 P0 | M | novo em `modules/common/`, `settings/`, `welcome/`, `TabletSystemActionRow.qml` |
| 7 | `back` nas bordas laterais como default da tablet | 🟠 P1 | P | `common/Config.qml:3713`, `TabletPoliciesDragHandler.qml` |
...
| 11 | Densidade da gaveta (~6–7 colunas, ícones maiores) | 🟠 P1 | P | `tablet/appDrawer/TabletAppDrawerContent.qml:52` |
...
| 14 | OSK reserva espaço quando aberta por auto-show | 🟠 P1 | P | `ii/onScreenKeyboard/`, `services/OskAutoShow.qml` |
...
| 19 | Teclado dividido (split keyboard) | 🟡 P2 | G | `ii/onScreenKeyboard/DeckContent.qml` |
...
| 29 | Sensibilidade da borda ajustável / por DPI | 🔵 P3 | P | `common/Config.qml:3678`, `TabletConfig.qml` |
| 30 | Haptics em toque longo e commit de gesto | 🔵 P3 | M | novo serviço |

**Sugestão de ordem de execução:** 1–6 (bloqueadores) → 7, 9, 11, 14, 17 (tudo pequeno, muito
retorno) → 4/8/10 fecham recentes → 12, 13, 15 → o resto.
...
B3 estar resolvido. *Recomendação: resolver B3, depois adotar o padrão Android e liberar o
segurar para recentes.*

**Q2 — Retrato / rotação continua fora?**
A decisão D5 diz "sem modo retrato". Ela faz todo sentido para laptops com touchscreen, que
não giram. Ela não faz sentido nenhuma para um tablet de verdade, que é o alvo declarado
deste documento — e o plano cita celulares como hardware alvo. Não estou pedindo para
reabrir; estou registrando que **é a maior divergência estrutural** em relação às duas
referências de produto, e que ela cresce de custo a cada superfície nova que assume paisagem.
...
- ❌ **Não adicionar propriedade em `modules/ii/` para resolver B1.** A OSK é usada pelas três
  famílias; ela deve ser **promovida** para `modules/common/` seguindo o procedimento do §3.3
  do plano — mover primeiro sem mudar nada, commit, depois parametrizar.
...
## 11. Como validar esta auditoria
