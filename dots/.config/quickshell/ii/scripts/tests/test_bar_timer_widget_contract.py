...
class BarTimerWidgetContractTest(unittest.TestCase):
    ...
    def test_expressive_surface_follows_bar_and_material_contracts(self):
        ...
        self.assertIn("theme.container", body)
        self.assertIn("qs.modules.common.functions", body)
        self.assertIn("ColorUtils.categoryOnColor(theme.container)", body)
        self.assertNotIn("color: theme.onContainer", body)
        self.assertIn("theme.accent", body)
        self.assertIn("theme.onAccent", body)
