...
/**
 * Monkeytype-shaped typing test, hosted inside the Overview search.
 *
 * The panel owns its own input (see `inputOwner` in SearchPanelRegistry), so
 * every keystroke here belongs to the test and never reaches the search query.
 * Settings and history are pages of this same panel rather than a trip to the
 * Settings window, because leaving the launcher would end the session.
 */
Item {
    id: root
    ...
    SearchPanelScaffold {
        ...
        primaryHint: ({ label: Translation.tr("Back"), keys: ["Esc"] })
        hints: [
                ...
                RowLayout {
                    ...
                    StyledText {
                        ...
                        text: root.page === "settings"
                            ? Translation.tr("Typing test settings") : Translation.tr("Score history")
