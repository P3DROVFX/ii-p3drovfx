...
AbstractBackgroundWidget {
    ...
    configEntryName: "resource_cpu_pill"
    ...
    readonly property var options: Config.options.background.widgets.resource_cpu_pill
    ...
    readonly property string titleText: {
        if (root.isWide && root.showDetails && root.cpuTemp > 0)
            return Translation.tr("CPU Usage") + " · " + Math.round(root.cpuTemp) + "°C";
        if (root.isWide)
            return Translation.tr("CPU Usage");
        return Translation.tr("CPU");
    }
    readonly property string valueText: Math.round(root.cpuUsage * 100) + "%"
        ...
        Item {
            ...
            Rectangle {
                ...
                Rectangle {
                    ...
                    width: Math.max(0, parent.width * Math.min(1.0, Math.max(0.0, root.cpuUsage)))
