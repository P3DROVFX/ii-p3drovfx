...
AbstractBackgroundWidget {
    ...
    readonly property real contentScale: (Config.options.background.widgets.scallop_dot_clock.widgetSize ?? 100) / 100.0
    ...
    readonly property bool cfgUseBlackBg:
        Config.ready ? (Config.options.background.widgets.scallop_dot_clock.useBlackBg ?? true) : true
    readonly property bool cfgEnableGlassReflection:
        Config.ready ? (Config.options.background.widgets.scallop_dot_clock.enableGlassReflection ?? false) : false
    readonly property bool cfgShowHourHand:
        Config.ready ? (Config.options.background.widgets.scallop_dot_clock.showHourHand ?? true) : true
    readonly property bool cfgShowMinuteBubble:
        Config.ready ? (Config.options.background.widgets.scallop_dot_clock.showMinuteBubble ?? true) : true
    readonly property bool cfgShowDots:
        Config.ready ? (Config.options.background.widgets.scallop_dot_clock.showDots ?? true) : true
    readonly property bool cfgBoldFont:
        Config.ready ? (Config.options.background.widgets.scallop_dot_clock.boldFont ?? true) : true
    ...
    Item {
        ...
        // 1. MaterialShape Cookie12Sided Background
        MaterialShape {
            id: clockBgShape
            anchors.fill: parent
